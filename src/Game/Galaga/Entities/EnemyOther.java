...
public class EnemyOther extends BaseEntity {
    ...
    int formationX, formationY, speed, centerCoolDown = 60, attackCooldown = random.nextInt(500) + 430;
    int timeAlive = 0;

    public EnemyOther(int x, int y, int width, int height, Handler handler, int row, int col) {
        super(x, y, width, height, Images.galagaEnemyOther[0], handler);
        this.row = row;
        this.col = col;
        BufferedImage[] idleAnimList = new BufferedImage[2];
        idleAnimList[0] = Images.galagaEnemyOther[0];
        idleAnimList[1] = Images.galagaEnemyOther[1];
    ...
    public void tick() {
        ...
        if (hit) {
            if (enemyDeath.end) {
                ...
                handler.getGalagaState().entityManager.playerShip.canSpawn[row][col] = false;
//                handler.getGalagaState().entityManager.playerShip.spawnTimer = random.nextInt(60*7);
                handler.getGalagaState().entityManager.playerShip.afterdeathSpawn("other",row,col);
        ...
        if (positioned) {
            ;
            //attackCooldown is a random number which will allow the bees to attack at different intervals
            attackCooldown--;
            if (attackCooldown <= 0) attacking = true;
        }
        //TODO TODO TODO
        if (attacking) {
            attackCooldown = random.nextInt(500);
            handler.getGalagaState().entityManager.lasers.add(new EnemyLaser((this.x + (width / 2)), this.y - 3, width / 5, height / 2,
                    Images.galagaPlayerLaser, handler, handler.getGalagaState().entityManager));
            attacking = false;
            return;
