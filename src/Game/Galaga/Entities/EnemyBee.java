    ...
    public void tick() {
        ...
        if (hit) {
            if (enemyDeath.end) {
                ...
                handler.getGalagaState().entityManager.playerShip.afterdeathSpawn("bee",row,col);
            }
            enemyDeath.tick();
        }
            ...
            if (!centered && Point.distance(x, y, handler.getWidth() / 2, handler.getHeight() / 2) > speed) {//reach center of screen
                ...
                if (centerCoolDown <= 0)
                {
                    if (Point.distance(x, y, formationX, formationY) > speed) {//reach center of screen
                        //changed the value from 6 to 3; makes them always position themselves
        ...
        if (attacking){
            //move towards players coordinates
            if(!hit) {
                y += speed;
                if (Point.distance(x, y, handler.getGalagaState().entityManager.playerShip.x,
                        handler.getGalagaState().entityManager.playerShip.y) > speed) {
