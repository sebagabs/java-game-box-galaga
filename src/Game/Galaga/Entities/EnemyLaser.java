...
public class EnemyLaser extends BaseEntity {
    ...
    public void tick() {
        if (!remove) {
            ...
            y += speed;
            bounds.y = y;
            //makes the laser be removed once out of bounds
            if(bounds.y > handler.getHeight()) remove=true;
            for (BaseEntity enemy : enemies.entities) {
                if (enemy instanceof EnemyOther || enemy instanceof EnemyBee || enemy instanceof EnemyLaser) {
                    continue;
                }
